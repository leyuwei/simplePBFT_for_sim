...
type Node struct {
	...
	filename    string
	loopId      int
	latestPPPST time.Time
	latestPPST  time.Time
	latestCST   time.Time
}
...
func NewNode(nodeID int, filename string) *Node {
	return &Node{
		nodeID,
		KnownNodes,
		ClientNode,
...
func (node *Node) handleRequest(payload []byte, sig []byte) {
	...
	node.mutex.Unlock()
	node.loopId += 1  
	node.latestPPPST = time.Now() 
	node.handleRecordTime(fmt.Sprintf("Loop:{%d} PPPST:{%d} ", node.loopId, time.Now().UnixNano()))
	logBroadcastMsg(hPrePrepare, prePrepareMsg)
	node.broadcast(msg)
}

func (node *Node) handleRecordTime(content string) {
	file, err := os.OpenFile(node.filename, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
...
func (node *Node) handlePrePrepare(payload []byte, sig []byte) {
	...
	node.mutex.Unlock()
	// prepare开始
	node.latestPPST = time.Now()
	node.handleRecordTime(fmt.Sprintf("PPPLT:{%d}ns{%d}ms ", node.latestPPST.UnixNano()-node.latestPPPST.UnixNano(), node.latestPPST.UnixMilli()-node.latestPPPST.UnixMilli()))
...
func (node *Node) handlePrepare(payload []byte, sig []byte) {
	...
	if sum >= limit {
		// Ready to record the duration of prepare
		node.latestCST = time.Now()
		node.handleRecordTime(fmt.Sprintf("PPLT:{%d} ", node.latestCST.UnixNano()-node.latestPPST.UnixNano()))
...
func (node *Node) handleCommit(payload []byte, sig []byte) {
	...
	if sum >= limit && node.msgLog.replyLog[commitMsg.Digest] == false {
		node.handleRecordTime(fmt.Sprintf("CLT:{%d} ", time.Now().UnixNano()-node.latestCST.UnixNano()))
		node.handleRecordTime(fmt.Sprintf("StoCLT:{%d} \n", time.Now().UnixNano()-node.latestPPPST.UnixNano()))
